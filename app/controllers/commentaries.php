...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['goComment'])){
    ...
    if($email === '' || $comment === ''){
        array_push($, "Не все поля заполнены!");
    }elseif (mb_strlen($comment, 'UTF8') < 50){
        array_push($, "Комментарий должен быть длинее 50 символов");
    }else{
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_comment'])){
    ...
    if($text === ''){
        array_push($, "Комментарий не имеет содержимого текста");
    }elseif (mb_strlen($text, 'UTF8') < 50){
        array_push($, "Количество символов внутри комментария меньше 50");
    }else{
        $com = [
            'comment' => $text,
            'status' => $publish
